...
async function loadSharedDb({ silent = false } = {}) {
  try {
    ...
    if (!silent)
      showToast("Shared server unavailable. Using this device cache.");
    return false;
  }
}
...
async function syncDbToServer(next, { replace = false } = {}) {
  try {
    ...
    state.serverOnline = false;
    showToast(
      "Saved on this device. Start the server to sync across phones/laptops.",
    );
    return false;
  }
}
...
async function refreshAdminData({ rerender = true } = {}) {
  ...
  if (
    changed &&
    document.getElementById("studentModal")?.classList.contains("open") &&
    ...
    es.onmessage = (ev) => {
      try {
        ...
        if (JSON.stringify(fresh) !== before) {
          ...
          // update login list for admins
          if (state.currentPage === "reports") renderAdminLogins();
        }
      } catch (_) {}
    };
    es.onerror = () => {
...
function showPage(name) {
  ...
  if (name === "track" && state.currentUser?.role === "admin") {
    showToast(
      "Admin accounts monitor users and cannot submit personal check-ins.",
    );
    name = "reports";
  }
...
function setRole(role, btn) {
  ...
  if (nameGroup)
    nameGroup.style.display = state.currentRole === "admin" ? "none" : "block";
}
...
async function login() {
  ...
  if (!validEmail(email) || !pass)
    return showToast("Please enter a valid email and password.");

  if (state.currentRole === "admin" || email === ADMIN_EMAIL) {
    if (email !== ADMIN_EMAIL || pass !== ADMIN_PASSWORD)
      return showToast("Invalid admin credentials.");
    state.currentUser = {
      id: "admin",
      name: "MindGuard Admin",
      email: ADMIN_EMAIL,
      role: "admin",
    };
    state.logs = [];
  ...
  // If server is online, prefer server-side auth so login events are recorded
  await loadSharedDb({ silent: true });
  if (state.serverOnline) {
    try {
      ...
      const resp = await fetch("/api/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, password: pass, ...meta }),
      });
      if (!resp.ok) {
        const err = await resp.json().catch(() => ({}));
        return showToast(err.error || "Invalid user email or password.");
      }
      const data = await resp.json();
      state.currentUser = publicUser(data.user);
      state.logs = userCheckins(data.user.id).reverse();
      saveSession(state.currentUser);
      updateNavUser();
      stopAdminRefresh();
      ...
      console.warn("Server auth failed, falling back to local:", err.message);
    }
  }

  // Local fallback (offline)
  const dataLocal = db();
  const user = dataLocal.users.find((u) => u.email.toLowerCase() === email);
  if (!user || user.password !== hashPassword(pass))
    return showToast("Invalid user email or password.");
...
async function register() {
  ...
  if (!first || !last || !validEmail(email) || !pass)
    return showToast("Please complete all required fields.");
  if (email === ADMIN_EMAIL)
    return showToast("That email is reserved for admin.");
  if (pass.length < 8)
    return showToast("Password must be at least 8 characters.");
  if (ecEmail && !validEmail(ecEmail))
    return showToast("Emergency contact email is invalid.");

  // Prefer server-side registration when online so admin records device info
  await loadSharedDb({ silent: true });
  if (state.serverOnline) {
    try {
      const meta = detectDevice();
      const resp = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({
          ...
          email,
          password: pass,
          emergencyContact: { name: ecName, email: ecEmail },
          ...meta,
        }),
      });
      if (!resp.ok) {
        const err = await resp.json().catch(() => ({}));
        return showToast(err.error || "Registration failed.");
      }
      const data = await resp.json();
      state.currentUser = publicUser(data.user);
      state.logs = [];
      ...
      console.warn(
        "Server register failed, falling back to local:",
        err.message,
      );
    }
  }
  ...
  const dataLocal = db();
  if (dataLocal.users.some((u) => u.email.toLowerCase() === email))
    return showToast("An account already exists. Please sign in.");
  ...
  const user = {
    ...
    emergencyContact: {
      name: ecName,
      email: ecEmail,
      relation: "Family/Counselor",
    },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  };
  dataLocal.users.push(user);
  await saveDb(dataLocal);
  state.currentUser = publicUser(user);
  state.logs = [];
  saveSession(state.currentUser);
  updateNavUser();
...
function initDashboard() {
  ...
  if (latest) updateDashboardScore(latest);
    ...
    ["hsMood", "hsSleep", "hsActivity", "hsSocial", "hsPsych"].forEach((id) =>
      setText(id, "-"),
    );
  }
  ...
  document.getElementById("dashTipsList").innerHTML = getWellnessTips(latest)
    .slice(0, 4)
...
function initAdminDashboard() {
  ...
  setText(
    "dashHealthStatus",
    `Registered users with data: ${summary.filter((r) => r.totalEntries > 0).length}`,
  );
  ...
  document.getElementById("alertList").innerHTML =
    db()
      ...
      .map(
        (a) =>
          `<div class="alert-item ${a.risk === "MEDIUM" ? "warn" : ""}"><div class="alert-item-icon">!</div><div class="alert-item-text"><div class="alert-item-name">${escapeHtml(a.userName || "User alert")}</div><div class="alert-item-desc">${escapeHtml(a.message)}</div></div><div class="alert-item-time">${formatDate(a.timestamp)}</div></div>`,
      )
      .join("") || '<div class="empty-state">No alerts yet.</div>';
  document.getElementById("dashTipsList").innerHTML =
    '<div class="tip-item">Use Reports to inspect a user profile, send alerts, export data, or delete a user.</div>';
...
function updateDashboardChartHeader(mode) {
  ...
  if (mode === "admin") {
    ...
    if (legend)
      legend.innerHTML =
        '<span class="legend-dot" style="background:#dc2626"></span>High risk<span class="legend-dot" style="background:#f59e0b"></span>Medium risk<span class="legend-dot" style="background:#059669"></span>Low risk';
  } else {
    if (title) title.textContent = "Wellbeing Problem Map";
    if (legend)
      legend.innerHTML =
        '<span class="legend-dot" style="background:#2563eb"></span>Mood<span class="legend-dot" style="background:#dc2626"></span>Stress<span class="legend-dot" style="background:#059669"></span>Sleep';
  }
}
...
function updateDashboardScore(log) {
  ...
  setText(
    "dashHealthStatus",
    `${log.risk} risk - ${log.risk === "LOW" ? "stable today" : "support recommended"}`,
  );
...
function updateStatCards(logsNewest) {
  ...
  const avg = (key) =>
    logsNewest.length
      ? logsNewest.reduce((a, l) => a + Number(l[key] || 0), 0) /
        logsNewest.length
      : 0;
  const values = [
  ...
  cards.forEach((card, i) => {
    ...
    if (trend)
      trend.textContent = logsNewest.length
        ? `${logsNewest.length} total check-ins`
        : "No entries yet";
  });
}
...
function renderPersonalAlerts() {
  ...
  el.innerHTML = alerts.length
    ? alerts
        .map(
          (a) =>
            `<div class="alert-item ${a.risk === "MEDIUM" ? "warn" : ""}"><div class="alert-item-icon">!</div><div class="alert-item-text"><div class="alert-item-name">${escapeHtml(a.title)}</div><div class="alert-item-desc">${escapeHtml(a.message)}</div></div><div class="alert-item-time">${formatDate(a.timestamp)}</div></div>`,
        )
        .join("")
    : '<div class="empty-state">No alerts yet. Keep checking in.</div>';
}

function renderAdminLogins() {
  const el = document.getElementById("loginList");
  if (!el) return;
  const logs = (db().logins || []).slice().reverse().slice(0, 200);
  if (!logs.length) {
    el.innerHTML = '<div class="empty-state">No login events yet.</div>';
    return;
  }
  el.innerHTML = logs
    .map((l) => {
      const who =
        l.userId === "admin" ? "Admin" : l.email || l.userId || "Unknown";
      const when = new Date(l.timestamp).toLocaleString();
      const succ = l.success ? "success" : "failed";
      const device = l.deviceType || (l.isMobile ? "mobile" : "desktop");
      const ip = l.ip || "";
      return `<div class="login-item" style="padding:0.45rem;border-bottom:1px solid #eee;font-size:0.9rem"><div><strong>${escapeHtml(who)}</strong> <span style="color:#666">(${escapeHtml(device)})</span></div><div style="color:#444">${escapeHtml(l.userAgent || "")}</div><div style="color:#666;font-size:0.85rem">${escapeHtml(ip)} · ${when} · <strong style="color:${l.success ? "#0b9b3b" : "#c62828"}">${succ}</strong></div></div>`;
    })
    .join("");
}

async function refreshLoginList() {
  await loadSharedDb({ silent: true });
  renderAdminLogins();
}
...
function avgMetric(logs, key) {
  return logs.length
    ? logs.reduce((sum, log) => sum + Number(log[key] || 0), 0) / logs.length
    : 0;
}

function readableChartOptions(
  yTitle,
  min = 0,
  max = 10,
  ...
  return {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (ctx) =>
            `${ctx.dataset.label}: ${valueLabel(ctx.parsed.y ?? ctx.raw)}`,
        },
      },
    },
    scales: {
...
function getTrendInsight(logs) {
  if (!logs.length)
    return {
      ...
      body: "Submit check-ins to see mood, stress, and sleep patterns.",
      tone: "info",
    };
  ...
  if (latest.risk === "HIGH")
    return {
      title: "Main concern: high latest risk",
      body: "Look for high stress, low mood, or low sleep on the same date.",
      tone: "risk",
    };
  if (avgStress >= 7)
    return {
      title: "Main concern: stress is staying high",
      body: `Average stress is ${avgStress.toFixed(1)}/10. Lower is better.`,
      tone: "risk",
    };
  if (avgSleep < 6)
    return {
      title: "Main concern: sleep is low",
      body: `Average sleep is ${avgSleep.toFixed(1)} hours. Aim for 7-9 hours when possible.`,
      tone: "warn",
    };
  if (avgMood < 5)
    return {
      title: "Main concern: mood is low",
      body: `Average mood is ${avgMood.toFixed(1)}/10. Extra support may help.`,
      tone: "warn",
    };
  return {
    title: "Pattern looks mostly stable",
    body: "Mood, stress, and sleep are not showing a strong warning pattern.",
    tone: "ok",
  };
}
...
function getRiskInsight(counts, total) {
  ...
  if (!total)
    return {
      ...
      body: "Risk distribution appears after check-ins are submitted.",
      tone: "info",
    };
  if (high)
    return {
      title: `${high} high-risk check-in${high === 1 ? "" : "s"}`,
      body: "Red means support is recommended soon. Open the user profile for details.",
      tone: "risk",
    };
  if (medium)
    return {
      title: `${medium} medium-risk check-in${medium === 1 ? "" : "s"}`,
      body: "Yellow means watch closely and encourage consistent check-ins.",
      tone: "warn",
    };
  return {
    title: `${low} low-risk check-in${low === 1 ? "" : "s"}`,
    body: "Green means the latest submitted data looks stable.",
    tone: "ok",
  };
}
...
function getSingleMetricInsight(id, logs, dataset) {
  if (!logs.length)
    return {
      title: "No data yet",
      body: "This chart will update after check-ins.",
      tone: "info",
    };
  ...
  if (id === "adminMoodChart") {
    return avg < 5
      ? {
          title: "Mood concern",
          body: `Average mood is ${avg.toFixed(1)}/10. Lower mood can signal emotional distress.`,
          tone: "warn",
        }
      : {
          title: "Mood is mostly stable",
          body: `Average mood is ${avg.toFixed(1)}/10. Higher is better.`,
          tone: "ok",
        };
  }
  if (id === "adminStressChart") {
    return avg >= 7
      ? {
          title: "Stress concern",
          body: `Average stress is ${avg.toFixed(1)}/10. Higher stress needs attention.`,
          tone: "risk",
        }
      : {
          title: "Stress is not extreme",
          body: `Average stress is ${avg.toFixed(1)}/10. Lower is better.`,
          tone: "ok",
        };
  }
  if (id === "adminSleepChart") {
    return avg < 6
      ? {
          title: "Sleep concern",
          body: `Average sleep is ${avg.toFixed(1)} hours. Low sleep can worsen mood and anxiety.`,
          tone: "warn",
        }
      : {
          title: "Sleep is readable",
          body: `Average sleep is ${avg.toFixed(1)} hours. Target range is 7-9.`,
          tone: "ok",
        };
  }
  return {
    title: dataset.label,
    body: "Read the line from left to right to see whether the problem is improving or worsening.",
    tone: "info",
  };
}
  ...
  trendChartInst = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        ...
        {
          label: "Stress - higher needs attention",
          data: logs.map((l) => l.stressLevel),
          borderColor: "#dc2626",
        ...
        {
          label: "Sleep hours - target 7-9",
          data: logs.map((l) => l.sleepHours),
          borderColor: "#059669",
    ...
    options: readableChartOptions(
      "Daily scores",
      0,
      12,
      (value) => `${value}/10 or hours`,
    ),
  });
}
...
function renderProgressDonut(logsNewest) {
  ...
  const count = logsNewest.filter(
    (l) => new Date(l.timestamp) >= new Date(Date.now() - 7 * 86400000),
  ).length;
  const pct = Math.min(100, Math.round((count / 7) * 100));
  renderChartInsight("donutChart", {
    title: logsNewest.length
      ? `${count} of 7 days completed this week`
      : "No check-ins this week",
    body: logsNewest.length
      ? "Regular check-ins make the risk graph more reliable."
      : "Complete a check-in to start seeing weekly progress.",
  ...
  if (legend)
    legend.innerHTML = `<div class="dl-item"><span class="dl-dot" style="background:#2563eb"></span>Completed (${pct}%)</div><div class="dl-item"><span class="dl-dot" style="background:#e2e8f0"></span>Missed (${100 - pct}%)</div>`;
}
...
function renderAdminReports() {
  const rows = adminRows().sort(
    (a, b) =>
      riskWeight(b.latestRisk) - riskWeight(a.latestRisk) ||
      new Date(b.lastCheckin) - new Date(a.lastCheckin),
  );
  document.querySelector("#page-reports .page-header h1").textContent =
    "Admin Dashboard";
  document.querySelector("#page-reports .page-sub").textContent =
    "Real registered users with submitted check-ins only";
...
function renderUserReportCharts(entriesNewest) {
  ...
  renderChartById(
    ...
    (inst) => (changeChartInst = inst),
    changeChartInst,
  );
  renderRiskPie(
    "riskChart",
    entriesNewest,
    (inst) => (riskChartInst = inst),
    riskChartInst,
  );
}
...
function openStudentModal(id) {
  if (state.currentUser.role !== "admin")
    return showToast("Only admin can open user profiles.");
  const data = db();
...
function exportReport() {
  ...
  const title = isAdmin
    ? "MindGuard Admin Report"
    : "MindGuard Personal Report";
...
function resetTrack() {
  ...
  goStep(1);
  document
    .querySelectorAll(".mcq-opt.active,.chip.active")
...
function updateEmoji(type, val) {
  const labels =
    val <= 3 ? "Very low" : val <= 5 ? "Low" : val <= 7 ? "Good" : "Great";
  setText(`${type}Val`, `${val} - ${labels}`);
}

function updateSleepVal(val) {
  setText("sleepVal", `${val} hours`);
}
function updateSocialVal(val) {
  setText(
    "socialVal",
    `${val} - ${val < 4 ? "Low interaction" : val < 8 ? "Moderate interaction" : "Connected"}`,
  );
}
function updateStressVal(val) {
  setText(
    "stressVal",
    `${val} - ${val > 7 ? "High stress" : val > 4 ? "Manageable stress" : "Calm"}`,
  );
}
...
function selectQual(el, groupId) {
  document
    .querySelectorAll(`#${groupId} .qual-opt`)
    .forEach((o) => o.classList.remove("active"));
  el.classList.add("active");
}
function selectAct(el) {
  document
    .querySelectorAll(".act-opt")
    .forEach((o) => o.classList.remove("active"));
  el.classList.add("active");
}
function selectMCQ(el, groupId, score) {
  document
    .querySelectorAll(`#${groupId} .mcq-opt`)
    .forEach((o) => o.classList.remove("active"));
  el.classList.add("active");
  state.mcqScores[groupId] = Number(score);
}
function calculateHealthScore() {
  ...
  const anxietyLevel =
    Math.max(
      Number(state.mcqScores.mq_anx1 || 0),
      Number(state.mcqScores.mq_anx2 || 0),
    ) *
      3 +
    1;
  const steps = Number(document.getElementById("stepsCount")?.value || 0);
  const active =
    document.querySelector(".act-opt.active .act-label")?.textContent ||
    "Moderate";
  const activityMap = {
    Sedentary: 20,
    Light: 45,
  ...
  const activityScore = Math.min(
    100,
    (activityMap[active] || 65) + Math.min(10, Math.floor(steps / 2000)),
  );
  const waterIntake = inferWaterIntake();
  const psychLoad = Object.values(state.mcqScores).reduce(
    (a, n) => a + Number(n || 0),
    0,
  );
  const sleepScore =
  ...
  const answeredScreening = Object.values(state.mcqScores).filter((v) =>
...
function inferWaterIntake() {
  ...
  return Math.max(
    ...
    Math.min(
      4,
      Math.round((1.8 + mood * 0.08 + sleep * 0.05 - stress * 0.04) * 10) / 10,
    ),
  );
}
...
async function submitLog() {
  if (!state.currentUser || state.currentUser.role !== "user")
    return showToast("Please sign in as a user.");
  await loadSharedDb({ silent: true });
  ...
  const checkin = {
    ...
    notes:
      ...
      document.getElementById("moodNotes").value.trim(),
  ...
  if (risk !== "LOW") {
    data.alerts.push({
      ...
      title: `${risk} Risk Check-in`,
      message: `${state.currentUser.name} submitted a ${risk} risk check-in.`,
      risk,
      timestamp: new Date().toISOString(),
      sentBy: "system",
    });
  ...
  showToast(
    state.serverOnline
      ? "Check-in saved and shared with admin."
  ...
  if (risk === "HIGH")
    setTimeout(
      () =>
        showAlertModal(
          "High Risk Detected",
          "Your latest check-in indicates high risk. Consider contacting your emergency support person or a qualified professional.",
        ),
      800,
    );
}
...
function getWellnessTips(log) {
  if (!log)
    return [
      "Complete your first check-in to unlock personalized guidance.",
      "Keep a consistent sleep and wake time.",
      "A short walk can improve mood and stress.",
    ];
  const tips = [];
  if (log.mood < 5)
    tips.push(
      "Low mood noted: try one small enjoyable activity and message someone you trust.",
    );
  if (log.sleepHours < 6)
    tips.push(
      "Sleep is low: aim for a screen-free wind-down and a consistent bedtime tonight.",
    );
  if (log.stressLevel > 7)
    tips.push(
      "Stress is high: use slow breathing for 3 minutes and reduce non-urgent tasks.",
    );
  if (log.anxietyLevel > 6)
    tips.push(
      "Anxiety is elevated: try grounding with 5 things you see, 4 you feel, 3 you hear.",
    );
  if (log.waterIntake < 2)
    tips.push("Hydration looks low: keep water nearby and sip steadily.");
  if ((log.activityScore || 0) < 50)
    tips.push(
      "Activity is low: a 10-minute walk is enough to restart momentum.",
    );
  return [
    ...tips,
    "If symptoms feel intense or unsafe, contact a mental health professional or crisis support.",
  ].slice(0, 8);
}
...
function initWellness() {
  const latest =
    state.currentUser?.role === "user"
      ? latestCheckin(state.currentUser.id)
      : null;
  const wc = document.getElementById("wellnessContent");
  if (wc) {
    wc.innerHTML = latest
      ? `<div class="wellness-result ${latest.risk === "LOW" ? "ok" : latest.risk === "MEDIUM" ? "warn" : "risk"}"><div class="wr-header"><div class="wr-icon">${latest.risk}</div><div><div class="wr-title">Latest risk: ${latest.risk}</div><div style="color:var(--text-2)">Score ${latest.healthScore}/100</div></div></div><div class="wr-tips">${getWellnessTips(
          latest,
        )
          .map((t) => `<div class="wr-tip">${t}</div>`)
          .join("")}</div></div>`
      : `<div class="dash-card" style="text-align:center"><h3>No check-in data yet</h3><p style="color:var(--text-2);margin:1rem">Complete a check-in to personalize this hub.</p><button class="btn-primary" onclick="showPage('track')">Start Check-In</button></div>`;
  }
...
function initProfile() {
  ...
  document.getElementById("pfEcEmail").value =
...
async function saveProfile() {
  ...
  user.emergencyContact = {
    name: document.getElementById("pfEcName").value.trim(),
    email: document.getElementById("pfEcEmail").value.trim(),
    relation: document.getElementById("pfEcRelation").value,
  };
...
function renderChartById(id, type, logs, datasets, setter, oldInst, max = 12) {
  ...
  if (id.startsWith("admin"))
    renderChartInsight(id, getSingleMetricInsight(id, logs, datasets[0]));
  const inst = new Chart(ctx, {
    ...
    options: readableChartOptions(
      max === 100 ? "Score out of 100" : "Daily value",
      0,
      max,
      (value) => (max === 100 ? `${value}/100` : `${value}`),
    ),
  });
  setter(inst);
}
...
function renderRiskPie(id, entries, setter, oldInst) {
  ...
  const counts = ["HIGH", "MEDIUM", "LOW"].map(
    (r) => entries.filter((e) => e.risk === r).length,
  );
  ...
  setter(
    new Chart(ctx, {
      ...
      data: {
        labels: [
          "High risk - needs support",
          "Medium risk - watch closely",
          entries.length ? "Low risk - stable" : "No data yet",
        ],
        datasets: [
          {
            data: displayCounts,
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: (ctx) =>
                entries.length
                  ? `${ctx.label}: ${ctx.raw} check-in${ctx.raw === 1 ? "" : "s"}`
                  : "No check-ins yet",
            },
          },
        },
...
function renderAdminVolumeChart(rows) {
  ...
  const mostEntries = rows.reduce(
    (top, row) => (row.totalEntries > (top?.totalEntries || 0) ? row : top),
    null,
  );
  ...
  renderChartInsight(
    ...
    rows.length
      ? {
          title: `${highRisk} high-risk user${highRisk === 1 ? "" : "s"} need attention`,
          body: `Tall bars mean more check-ins. ${mostEntries?.name || "A user"} has the most data for review.`,
          tone: highRisk ? "risk" : "ok",
        }
      : {
          title: "No user check-ins yet",
          body: "Graphs will appear after users submit check-ins.",
          tone: "info",
        },
  );
  changeChartInst = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        {
          label: "Number of check-ins submitted",
          data: rows.map((r) => r.totalEntries),
    ...
    options: readableChartOptions(
      "Check-ins per user",
      0,
      Math.max(5, ...rows.map((r) => r.totalEntries)) + 1,
      (value) => `${value} check-ins`,
    ),
  });
}
...
function renderAdminDetailCharts(entriesNewest) {
  ...
  renderChartById(
    ...
    (inst) => (adminSleepChart = inst),
    adminSleepChart,
  );
  renderRiskPie(
    "adminRiskChart",
    entriesNewest,
    (inst) => (adminRiskChart = inst),
    adminRiskChart,
  );
}
...
function newChat() {
  document.getElementById("chatMessages").innerHTML =
    '<div class="msg ai"><div class="msg-avatar">AI</div><div class="msg-bubble"><p>New conversation started. How can I support you today?</p></div></div>';
  state.chatHistory = [];
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (restored)
    showPage(state.currentUser.role === "admin" ? "reports" : "dashboard");
  ...
  document.addEventListener("visibilitychange", () => {
    if (!document.hidden) {
      if (state.currentUser?.role === "admin") {
        ...
        if (
          !state.sseSource ||
          state.sseSource.readyState === EventSource.CLOSED
        ) {
          startAdminRefresh();
        }
        refreshAdminData();
